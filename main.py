...
class IthoRemoteRFT:
    ...
    def __init__(self, port='COM3', baud=115200):
        ...
        self.regex_pattern = "(?P<RSSI>\\d{3}) (?P<VERB> I|RQ|RP| W) (?P<SEQNR>---|\\d{3}) " \
                             "(?P<ADDR1>--:------|\\d{2}:\\d{6}) " \
                             "(?P<ADDR2>--:------|\\d{2}:\\d{6}) " \
                             "(?P<ADDR3>--:------|\\d{2}:\\d{6}) " \
        ...
        unit_address_info = f"paired to {self.unit_address}" if self.unit_address is not None else "not paired"
    ...
    def load_settings(self):
        try:
            with open('settings.json', 'r') as f:
                ...
                self.remote_address = settings.get('remote_address', self.remote_address)
    ...
    def _parse_status(self, payload):
        ...
        outdoor_humidity = outdoor_humidity_raw if 0 <= outdoor_humidity_raw <= 100 else None
        indoor_humidity_raw = int(b[12:14], 16)
        indoor_humidity = indoor_humidity_raw if 0 <= indoor_humidity_raw <= 100 else None

        # Parse HRU channel temperatures (°C)
        exhaust_temperature_raw = int(b[14:18], 16)
        exhaust_temperature_raw -= 0x10000 if exhaust_temperature_raw & 0x8000 else 0  # Parse signed int
        exhaust_temperature_raw = (exhaust_temperature_raw / 100.0)
        exhaust_temperature = exhaust_temperature_raw if -273.15 <= exhaust_temperature_raw <= 327.66 else None

        supply_temperature_raw = int(b[18:22], 16)
        supply_temperature_raw -= 0x10000 if supply_temperature_raw & 0x8000 else 0  # Parse signed int
        supply_temperature_raw = (supply_temperature_raw / 100.0)
        supply_temperature = supply_temperature_raw if -273.15 <= supply_temperature_raw <= 327.66 else None

        indoor_temperature_raw = int(b[22:26], 16)
        indoor_temperature_raw -= 0x10000 if indoor_temperature_raw & 0x8000 else 0  # Parse signed int
        indoor_temperature_raw = (indoor_temperature_raw / 100.0)
        indoor_temperature = indoor_temperature_raw if -273.15 <= indoor_temperature_raw <= 327.66 else None

        outdoor_temperature_raw = int(b[26:30], 16)
        outdoor_temperature_raw -= 0x10000 if outdoor_temperature_raw & 0x8000 else 0  # Parse signed int
        outdoor_temperature_raw = (outdoor_temperature_raw / 100.0)
        outdoor_temperature = outdoor_temperature_raw if -273.15 <= outdoor_temperature_raw <= 327.66 else None

        # Capability flags (bitmask)
        capability_flags_raw = int(b[30:34], 16)
        capability_names = [
            "Off", "Away", "Timer", "Boost",
            "Auto", "Speed 4", "Speed 5", "Speed 6",
            "Speed 7", "Speed 8", "Speed 9", "Speed 10",
            "Night", "Reserved", "Post Heater", "Pre Heater"
        ]
        set_capabilities = [capability_names[i] for i in range(16) if capability_flags_raw & (1 << (15 - i))]
        capability_flags = ", ".join(set_capabilities) + " Capable"

        # Parse bypass position (%)
        bypass_position_raw = (int(b[34:36], 16) / 2)
        bypass_position = bypass_position_raw if 0.0 <= bypass_position_raw <= 100.0 else None
        ...
        flags_active_speed_mode = active_speed_mode_mapping.get(flags_active_speed_mode_raw, "Unknown")

        # Parse fan speed inlet/exhaust (%)
        exhaust_fan_speed_raw = (int(b[38:40], 16) / 2)
        exhaust_fan_speed = exhaust_fan_speed_raw if 0.0 <= exhaust_fan_speed_raw <= 100.0 else None
        inlet_fan_speed_raw = (int(b[40:42], 16) / 2)
        inlet_fan_speed = inlet_fan_speed_raw if 0.0 <= inlet_fan_speed_raw <= 100.0 else None
        ...
        parsed_data_filtered = {key: value for key, value in parsed_data.items() if value is not None}
